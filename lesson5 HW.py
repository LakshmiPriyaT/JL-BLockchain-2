...
class Block(object):
  ...
  def __init__(self):
    ...
    self.count = 0
    self.new_block(
      previous_hash="No previous Hash. Since this is the first block.")
  ...
  def new_block(self, previous_hash=None):
    ...
    block = {
      ...
      # 'Hex':  self.hash(self.chain) or self.hash(self.chain[-1])
    }
    self.new_transactions = []
    self.count = self.count + 1
    self.chain.append(block)
    # block['Current hash'] = self.hash(self.chain[-1])
...
# Write Task 02 code from here
print('Previous hash code of Genesis block:',
      blockchain.chain[0]['previous_hash'])
...
# Write Task 03 code from here
if blockchain.chain[0]['Current_hash'] == blockchain.chain[1]['previous_hash']:
  print(
    "The Hash Of current Block Matches with Previous Block...Hence Transcation Verified..!"
  )
else:
  print(
    "Oh..No..The Hash Of current Block Does not Matches with Previous Block")
